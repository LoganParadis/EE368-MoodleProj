...
class tool_bulkenrol_tracker {
     ...
     */
    protected $columns = array('line', 'result', 'user_display', 'course_display', 'role_display', 'status');
     ...
     * Output the results.
     *
     * @param int $total total bulkusers.
     * @param int $enrolled count of bulkusers enrolled.
     * @param int $errors count of errors.
     * @return void
     */
    public function results($total, $enrolled, $errors) {
        if ($this->outputmode == self::NO_OUTPUT) {
            return;
        }

        $message = array(
            get_string('bulkuserstotal', 'tool_bulkenrol', $total),
            get_string('bulkusersenrolled', 'tool_bulkenrol', $enrolled),
            get_string('bulkuserserrors', 'tool_bulkenrol', $errors)
        );
    ...
    public function output($line, $outcome, $status, $data) {
        ...
        if ($this->outputmode == self::OUTPUT_PLAIN) {
            $message = array(
                $line,
                $outcome ? 'OK' : 'NOK',
                isset($data['user_display']) ? $data['user_display'] : '',
                isset($data['course_display']) ? $data['course_display'] : '',
                isset($data['role_display']) ? $data['role_display'] : ''
            );
            ...
            echo html_writer::tag('td', isset($data['user_display']) ? $data['user_display'] : '', array('class' => 'c' . $ci++));
            echo html_writer::tag('td', isset($data['course_display']) ? $data['course_display'] : '', array('class' => 'c' . $ci++));
            echo html_writer::tag('td', isset($data['role_display']) ? $data['role_display'] : '', array('class' => 'c' . $ci++));
    ...
    public function start() {
        ...
        if ($this->outputmode == self::OUTPUT_PLAIN) {
            ...
            echo html_writer::tag('th', get_string('user_display', 'tool_bulkenrol'), array('class' => 'c' . $ci++, 'scope' => 'col'));
            echo html_writer::tag('th', get_string('course_display', 'tool_bulkenrol'), array('class' => 'c' . $ci++, 'scope' => 'col'));
            echo html_writer::tag('th', get_string('role_display', 'tool_bulkenrol'), array('class' => 'c' . $ci++, 'scope' => 'col'));
