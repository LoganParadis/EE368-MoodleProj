...
/**
 * Bulk enrol users registration script from a comma separated file.
 *
...
if (empty($importid)) {
    ...
    if ($form1data = $mform1->get_data()) {
        ...
        $content = $mform1->get_file_content('bulkuserfile');
...
if ($form2data = $mform2->is_cancelled()) {
    ...
    if (!empty($form1data)) {
        ...
        $options = array(
            'mode_user' => tool_bulkenrol_processor::MODE_USER_EMAIL,
            'mode_course' => tool_bulkenrol_processor::MODE_COURSE_SHORTNAME,
            'mode_role' => tool_bulkenrol_processor::MODE_ROLE_SHORTNAME);
    }
    $processor = new tool_bulkenrol_processor($cir, $options);
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('bulkenrolspreview', 'tool_bulkenrol'));
    $array = $processor->preview($previewrows, new tool_bulkenrol_tracker(tool_bulkenrol_tracker::OUTPUT_HTML));
    $mform2->display();
}
